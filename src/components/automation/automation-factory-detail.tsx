...
import { getCategoryById, parseModels } from "@/lib/catalog";
import { getModels } from "@/lib/data";
import { industryTagLabel } from "@/lib/industry-tags";
import type { Factory } from "@/lib/schemas";

/**
 * Карточка завода для "Автоматического оборудования" — сестра
 * ProductionFactoryDetail, но для сегмента automation: конвейерные
 * весы, сортировщики, рентген-контроль и линии убоя без
 * роботизированного манипулятора. Держим отдельно от продакшн-детали,
 * чтобы ссылки/подписи не путали "роботизированное" с "автоматическим".
 */
export async function AutomationFactoryDetail({ factory, locale }: { factory: Factory; locale: string }) {
  const t = await getTranslations({ locale, namespace: "catalog" });
  const tAutomation = await getTranslations({ locale, namespace: "automation" });
  ...
  const categories = (
    await Promise.all(factory.categories.map((id) => getCategoryById(id)))
  ).filter((c): c is NonNullable<typeof c> => Boolean(c) && c?.segment === "automation");
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {categories.length > 0 && (
        <div>
          <h2 className="text-heading-sm">{tAutomation("factoryEquipmentTitle")}</h2>
          <div className="mt-3 flex flex-wrap gap-2">
            {categories.map((category) => (
              ...
              >
                {isEn ? category.name_en : category.name_ru}
              </Link>
            ))}
          </div>
        </div>
