"""Shared Telegram-sending helper for scripts/publish-pending.py.

The VPS's DNS intermittently fails to resolve or route to some of
Telegram's IPs while others work fine — a leftover of partial
ISP/hosting-level filtering (confirmed while setting up the same
mechanism for globaltechtour.ru on this same VPS). Connecting straight
to a verified IP sidesteps that: the TLS handshake still verifies
against the real hostname (SNI + cert check), and the Host header is
still the real domain, only the TCP destination is pinned — the same
trick as `curl --resolve`. Falls back to normal DNS resolution in case
this specific IP stops working later.
"""
import http.client
import json
import socket
